			...
			<el-form class="center-form-pv" :style='{"width":"100%","margin":"0 10px 20px"}' :inline="true" :model="searchForm">
				<el-row :style='{"width":"100%","padding":"10px","alignItems":"center","flexWrap":"wrap","background":"none","display":"flex"}' >
					<div :style='{"alignItems":"center","margin":"0 10px 0 0","display":"flex"}'>
						<label :style='{"margin":"0 5px 0 0","whiteSpace":"nowrap","color":"#fff","display":"inline-block","width":"auto","lineHeight":"40px","fontSize":"15px","fontWeight":"500","height":"40px"}' class="item-label">维修标题</label>
						<el-input v-model="searchForm.weixiubiaoti" placeholder="维修标题" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"alignItems":"center","margin":"0 10px 0 0","display":"flex"}'>
						<label :style='{"margin":"0 5px 0 0","whiteSpace":"nowrap","color":"#fff","display":"inline-block","width":"auto","lineHeight":"40px","fontSize":"15px","fontWeight":"500","height":"40px"}' class="item-label">故障部件</label>
						<el-input v-model="searchForm.guzhangbujian" placeholder="故障部件" @keydown.enter.native="search()" clearable></el-input>
					</div>
				...
				<el-row class="actions" :style='{"padding":"10px","margin":"20px 0","flexWrap":"wrap","background":"none","flexDirection":"row-reverse","display":"flex","width":"100%","justifyContent":"flex-start"}'>
					<el-button class="add" v-if="isAuth('weixiuzhishiku','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia14" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","height":"38px"}'></span>
						创建
					</el-button>
					<el-button class="del" v-if="isAuth('weixiuzhishiku','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='false' :sortable='false'  
						...
						<template slot-scope="scope">
							{{scope.row.weixiubiaoti}}
						</template>
					</el-table-column>
					<el-table-column  :resizable='false' prop="fengmian" width="200" label="封面">
						<template slot-scope="scope">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('weixiuzhishiku','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								查看
							</el-button>
							<el-button class="edit" v-if=" isAuth('weixiuzhishiku','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								编辑
							</el-button>
							...
							<el-button class="del" v-if="isAuth('weixiuzhishiku','删除') " type="primary" @click="deleteHandler(scope.row.id )">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								删除
							</el-button>
						</template>
		...
		methods: {
			...
			getDataList() {
				...
				let params = {
 				...
 				if(this.searchForm.weixiubiaoti!='' && this.searchForm.weixiubiaoti!=undefined){
					params['weixiubiaoti'] = '%' + this.searchForm.weixiubiaoti + '%'
				}
 				if(this.searchForm.guzhangbujian!='' && this.searchForm.guzhangbujian!=undefined){
					params['guzhangbujian'] = '%' + this.searchForm.guzhangbujian + '%'
				}
				let user = JSON.parse(this.$storage.getObj('userForm'))
				this.$http({
					url: "weixiuzhishiku/page",
					method: "get",
					params: params
				}).then(({ data }) => {
					if (data && data.code === 0) {
