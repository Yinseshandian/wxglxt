		...
		>
			<template >
				...
				<el-form-item class="select" v-if="type!='info'"  label="维修类型" prop="weixiuleixing" >
					<el-select :disabled="ro.weixiuleixing" v-model="ruleForm.weixiuleixing" placeholder="请选择维修类型" >
						<el-option
				...
				<el-form-item class="upload" v-if="type!='info' && !ro.weixiutupian" label="维修图片" prop="weixiutupian" >
					<file-upload
						tip="点击上传维修图片"
						action="file/upload"
						:limit="3"
				...
				<el-form-item class="upload" v-else-if="ruleForm.weixiutupian" label="维修图片" prop="weixiutupian" >
					<img v-if="ruleForm.weixiutupian.substring(0,4)=='http'&&ruleForm.weixiutupian.split(',w').length>1" class="upload-img" style="margin-right:20px;" v-bind:key="index" :src="ruleForm.weixiutupian" width="100" height="100">
					<img v-else-if="ruleForm.weixiutupian.substring(0,4)=='http'" class="upload-img" style="margin-right:20px;" v-bind:key="index" :src="ruleForm.weixiutupian.split(',')[0]" width="100" height="100">
				...
				<el-form-item class="input" v-if="type!='info'"  label="确认备注" prop="querenbeizhu" >
					<el-input v-model="ruleForm.querenbeizhu" placeholder="确认备注" clearable  :readonly="ro.querenbeizhu"></el-input>
				</el-form-item>
				<el-form-item v-else class="input" label="确认备注" prop="querenbeizhu" >
					<el-input v-model="ruleForm.querenbeizhu" placeholder="确认备注" readonly></el-input>
				</el-form-item>
			</template>
			<el-form-item v-if="type!='info'"  label="维修记录" prop="weixiujilu" >
				<editor 
					...
					class="editor"
					myQuillEditor="weixiujilu"
					action="file/upload">
				</editor>
			</el-form-item>
			<el-form-item v-else-if="ruleForm.weixiujilu" label="维修记录" prop="weixiujilu" >
				<span class="text ql-snow ql-editor" v-html="ruleForm.weixiujilu"></span>
			</el-form-item>
			...
			init(id,type) {
				...
				if(this.type=='info'||this.type=='else'||this.type=='msg'){
					...
					for (var o in obj){
						...
						if(o=='querenbeizhu'){
							this.ruleForm.querenbeizhu = obj[o];
							this.ro.querenbeizhu = true;
							continue;
						}
						if(o=='weixiujilu'){
				...
				this.$http({
					...
					if (data && data.code === 0) {
						...
						if(((json.weixiushenfen!=''&&json.weixiushenfen) || json.weixiushenfen==0) && this.$storage.get("role")!="管理员"){
							this.ruleForm.weixiushenfen = json.weixiushenfen
							this.ro.weixiushenfen = true;
						}
					} else {
			...
			info(id) {
				this.$http({
					...
					if (data && data.code === 0) {
						...
						this.ruleForm.weixiujilu = this.ruleForm.weixiujilu.replace(reg,'../../../wxglxt/upload');
					} else {
						this.$message.error(data.msg);
					}
				});
